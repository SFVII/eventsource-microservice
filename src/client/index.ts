...
class EventsPlugin<DataModel, Contributor> extends DataTreated {
    ...
    private async InitStreamWatcher() {
        ...
        await this.client.setStreamMetadata(this.streamName, metadata);
        if (this.stream) {
    ...
    private EventMiddlewareEmitter(data: ModelEventWrapper, method: string, typeOrigin?: string, contributor?: IContributor<Contributor>, catchStreamResult?: string, specificQuery?: any): Promise<{
        payload: IEventResponseError | IEventResponseSuccess<any> | null,
        error?: any,
        requestId: string
    }> {
