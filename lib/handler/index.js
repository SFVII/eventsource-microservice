...
class EventHandler {
    constructor(EvenStoreConfig, dispatcherList, triggerOnComplete = [], globalStreamName = 'global-event-handler') {
    ...
    async init() {
        ...
        for (const stream of this.streamList) {
            console.log('subscribe to stream > %s', stream);
            this.StartRevision[stream] = global_1.START;
    ...
    SubscribeToPersistent(streamName) {
        return this.client.subscribeToPersistentSubscription(this.globalStreamName, streamName, {
            bufferSize: 10
        });
    }
    async CreatePersistentSubscription(streamName) {
        try {
            await this.client.createPersistentSubscription(this.globalStreamName, streamName, (0, global_1.persistentSubscriptionSettingsFromDefaults)({
                startFrom: this.StartRevision[streamName],
                resolveLinkTos: true
            }), { credentials: this.credentials });
            return true;
