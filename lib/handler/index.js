...
const listStreams = async (client) => {
    const iterator = client.readAll({
        direction: 'backwards',
        maxCount: Number.MAX_SAFE_INTEGER,
        resolveLinkTos: false,
        fromPosition: global_1.START
    });
...
class EventHandler {
    constructor(EvenStoreConfig, streamList, triggerOnComplete = [], group = 'global-event-handler') {
        ...
        this.init().then(() => {
            setInterval(() => {
                this.init().catch((e) => console.error('Interval Check Like Soron Eyes for Subscribing new queue errored', e));
            }, 1000 * 60);
        }).catch((err) => {
    ...
    async init() {
        ...
        this.StartRevision = {};
        this.stream = {};
        const existingStream = await listStreams(this.client);
        console.log('Existing Stream', existingStream);
        if (existingStream) {
            const filteredEyes = existingStream.filter((e) => this.streamList.includes(e));
            const keepNotRegistered = filteredEyes.filter((e) => Object.keys(this.stream).indexOf(e) === -1);
