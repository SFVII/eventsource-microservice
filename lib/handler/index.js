...
class EventHandler {
    ...
    async handler(event) {
        const eventParser = new CommonResponse_1.EventParser(event);
    ...
    async CreatePersistentSubscription(streamName) {
        try {
            ...
            // @ts-ignore
            (0, db_client_1.persistentSubscriptionToStreamSettingsFromDefaults)({ startFrom: global_1.END }), { credentials: this.credentials });
            return true;
        }
        catch (err) {
