...
class EventHandler {
    constructor(EvenStoreConfig, streamList, triggerOnComplete = [], group = 'dispatch') {
    ...
    async handler(event) {
        if (Array.isArray(event.metadata.causationRoute)) {
            ...
            console.log('[%s] Incoming event (route > %s) \t\tnext event (%s)', event.metadata.state.toUpperCase(), nextRoute, (Routes && Routes.length ? Routes[0] : 'COMPLETED'));
            if (nextRoute) {
                if (event.metadata && event.metadata.state === "error") {
                    const template = this.template(event.type, event.data, {
                        $correlationId: event.metadata.$correlationId,
