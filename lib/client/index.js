...
class EventsPlugin extends DataTreated {
    ...
    async CreatePersistentSubscription(streamName) {
        ...
        console.log('FUKKKKKKKK', streamName, this.group, 
        // @ts-ignore
        (0, db_client_1.persistentSubscriptionToStreamSettingsFromDefaults)(), { credentials: this.credentials });
        const status = await this.client.createPersistentSubscriptionToStream(streamName, this.group, 
        // @ts-ignore
        (0, db_client_1.persistentSubscriptionToStreamSettingsFromDefaults)(), { credentials: this.credentials }).catch(async (err) => {
            console.log('Err', err);
