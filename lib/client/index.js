...
class EventsPlugin {
    constructor(EvenStoreConfig, streamName, methods, causationRoute) {
        ...
        for (const method of this.methods) {
            // @ts-ignore
            this[method] = async (data, contributor, typeOrigin, streamName, causationRoute) => {
                const { payload, requestId } = await this.EventMiddlewareEmitter(data, method, streamName, causationRoute, typeOrigin, contributor);
                return {
                    data: payload,
                    ack: this.delivered(requestId, method, payload, typeOrigin, streamName ? streamName : this.streamName, [])
                        .bind(this)
                };
            };
        }
    }
    delivered(requestId, method, payload, typeOrigin, streamName, causationRoute) {
    ...
    async EventMiddlewareEmitter(data, method, _streamName, causationRoute, typeOrigin, contributor) {
