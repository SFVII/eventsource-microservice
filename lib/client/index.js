...
class EventsPlugin {
    constructor(EvenStoreConfig, streamName, methods, causationRoute) {
        ...
        for (const method of this.methods) {
            ...
            // this.create({...}, {id: xxxx}, event.typeOrigin)
                ...
                const { payload, requestId } = await this.EventMiddlewareEmitter(data, method, this.streamName, typeOrigin, contributor);
                return {
                    data: payload,
                    ack: this.delivered(requestId, method, payload, typeOrigin, this.streamName, contributor, this.streamName)
                        .bind(this)
                };
            };
        }
    ...
    async EventMiddlewareEmitter(data, method, _streamName, typeOrigin, contributor) {
    ...
    async appendToStream(streamName, template) {
        return await this.client.appendToStream(streamName || this.streamName, [template])
            .catch((err) => {
