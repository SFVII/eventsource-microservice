...
class DataTreated {
    ...
    async find(IdEvent, catchStreamResult = null, retry = 0) {
        ...
        if (!this.list.length) {
            ...
            const lookup = this.list.find((doc) => !catchStreamResult ? doc.id === IdEvent : (catchStreamResult === doc.causation && doc.id === IdEvent));
...
class EventsPlugin extends DataTreated {
    constructor(EvenStoreConfig, streamName, methods, causationRoute) {
        ...
        for (const method of this.methods) {
            // @ts-ignore
            this[method] = async (data, contributor, typeOrigin, catchStreamResult) => {
                const { payload, requestId, error } = await this.EventMiddlewareEmitter(data, method, typeOrigin, contributor, catchStreamResult)
                    // @ts-ignore
                    .catch((err) => {
                    return {
                        payload: null,
    ...
    EventMiddlewareEmitter(data, method, typeOrigin, contributor, catchStreamResult) {
        return new Promise(async (resolve, reject) => {
            ...
            if (this.exist(requestId)) {
                const event = await this.find(requestId, catchStreamResult);
                if (event && event.data) {
                    return resolve({
                        payload: event?.data,
                        requestId
            ...
            const event = await this.find(requestId, catchStreamResult, 0);
            if (event)
