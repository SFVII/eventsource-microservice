...
class EventsPlugin extends DataTreated {
    ...
    async CreatePersistentSubscription(streamName) {
        ...
        const status = await this.client.createPersistentSubscription(streamName, this.group, (0, global_1.persistentSubscriptionSettingsFromDefaults)({
            startFrom: global_1.START,
            resolveLinkTos: true
        }), { credentials: this.credentials }).catch((err) => {
            console.log('Err', err);
