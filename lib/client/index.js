...
class EventsPlugin extends DataTreated {
    ...
    async CreatePersistentSubscription(streamName) {
        ...
        const status = await this.client.createPersistentSubscriptionToStream(streamName, streamName, 
        // @ts-ignore
        (0, db_client_1.persistentSubscriptionToStreamSettingsFromDefaults)({
            startFrom: db_client_1.END,
            resolveLinkTos: true
        }), { credentials: this.credentials }).catch(async (err) => {
            console.log('Err', err);
            const error = (err ? err.toString() : "").toLowerCase();
            if (error.includes('EXIST') || error.includes('exist')) {
                const x = await this.client.updatePersistentSubscriptionToStream(streamName, streamName, (0, db_client_1.persistentSubscriptionToStreamSettingsFromDefaults)({
                    startFrom: db_client_1.END,
                    resolveLinkTos: true,
