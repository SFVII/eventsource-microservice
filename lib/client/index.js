...
class DataTreated {
    ...
    exist(IdEvent) {
        return list.findIndex((doc) => doc.id === IdEvent) > -1;
    }
    add(entry) {
        if (this.clear_process) {
            ...
            const index = list.findIndex((doc) => entry.id == doc.id);
    ...
    async find(IdEvent, retry = 0) {
        ...
        if (!list.length) {
            ...
            const lookup = list.find((doc) => doc.id === IdEvent);
    ...
    clearOldFile() {
        setInterval(() => {
            ...
            list = list.filter((doc) => doc.date.getTime() >= limit.getTime()) || [];
